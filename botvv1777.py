...
def init_database():
    """Initialize SQLite database with auto-update capability - FIXED VERSION"""
    try:
        conn = sqlite3.connect(DB_NAME)
        cursor = conn.cursor()
        ...
        # Create channel_verification table - FIXED: Add this table
        cursor.execute('''
...
def force_recreate_database():
    """Force recreate all database tables - USE THIS IF STILL HAVE ERRORS"""
    try:
        conn = sqlite3.connect(DB_NAME)
        cursor = conn.cursor()
        ...
        tables = [
            'users', 'user_settings', 'bet_history', 'pending_bets', 
            'bot_sessions', 'bs_patterns', 'channel_verification',
            'sl_patterns', 'sl_bet_sessions', 'formula_patterns'
        ]
        ...
        init_database()
        print("✅ Database force recreated successfully")
        
    except Exception as e:
        print(f"❌ Error force recreating database: {e}")
...
def get_main_keyboard(user_id=None):
    """Get main keyboard with localized text - ONLY 777 LOGIN"""
    if user_id:
...
async def stop_bot_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Clear pending bets with error handling
        try:
            conn = sqlite3.connect(DB_NAME)
            cursor = conn.cursor()
            cursor.execute('DELETE FROM pending_bets WHERE user_id = ?', (user_id,))
            conn.commit()
            conn.close()
            print(f"✅ Cleared pending bets for user {user_id}")
        except Exception as e:
            print(f"⚠️ Could not clear pending bets: {e}")
        ...
        await update.message.reply_text(
            ...
            "Some cleanup operations may have failed, but the bot is no longer running."
        )
