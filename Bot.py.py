...
# --- ADD THIS LINE ---
# အသုံးပြုခွင့်ပေးထားတဲ့ Telegram User IDs
# Admin ရဲ့ ID ကို အရင်ထည့်ပါ (ဥပမာ: 6328953001)
ALLOWED_USER_IDS = [6328953001]  # ဒီနေရာမှာ Admin ရဲ့ နဲ့ အသုံးပြုခွင့်ပေးထားတဲ့ တကယ့် User IDs တွေကို ထည့်ပါ (ဥပမာ: [6328953001, 987654321])
# ----------------------
...
# --- ADD THESE FUNCTIONS ---
def load_allowed_users_from_db():
    ...
    try:
        ...
        logger.error(f"Error loading allowed users from DB: {e}")
...
def add_user_to_db(user_id):
    ...
    try:
        conn = sqlite3.connect(DB_NAME)
        cursor = conn.cursor()
        cursor.execute('INSERT OR REPLACE INTO allowed_users (user_id) VALUES (?)', (user_id,))
        conn.commit()
        conn.close()
        return True
    except Exception as e:
...
def remove_user_from_db(user_id):
    """Remove user from allowed_users table"""
    try:
        conn = sqlite3.connect(DB_NAME)
        cursor = conn.cursor()
        ...
        logger.error(f"Error removing user {user_id} from DB: {e}")
        return False
# --------------------------
...
def init_database():
    ...
    try:
        ...
        ''')
        # Create pending_bets table - FIXED: Add this table
        cursor.execute('''
...
def remove_pending_bet(user_id, platform, issue):
    """Remove pending bet after result is known - WITH ERROR HANDLING"""
    try:
        conn = sqlite3.connect(DB_NAME)
        cursor = conn.cursor()
        # Check if table exists first
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='pending_bets'")
        table_exists = cursor.fetchone()
        if not table_exists:
            print("DEBUG: pending_bets table doesn't exist, creating...")
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS pending_bets (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    user_id INTEGER,
            ...
            ''')
            conn.commit()
            print("DEBUG: pending_bets table created")
            conn.close()
            return True
...
def get_localized_message(message_key, language='english'):
    ...
    messages = {
        ...
        'burmese': {
            'welcome': "အိုင်တီ ဘော့ခ်\nကြိုဆိုပါသည်!",
            'login_success': "လော့ဂ်အင် အောင်မြင်ပါသည်!",
            'bet_placed': "ချက်ထားမှု အောင်မြင်ပါသည်!",
            'balance': "ငွေပမာဏ",
            'language_set': "ဘာသာစကားကို မြန်မာဘာသာသို့ ပြောင်းလိုက်ပါပြီ\nအားလုံးကို မြန်မာဘာသာဖြင့် ပြပေးမည်ဖြစ်ပါသည်။",
            'choose_language': "သင့်ဘာသာစကားကို ရွေးချယ်ပေးပါ\nကျေးဇူးပြု၍ သင့်နှစ်သက်ရာ ဘာသာစကားကို ရွေးချယ်ပေးပါ:",
            'bot_settings': "ဘော့ခ် ဆက်တင်",
            'random_big': "ကျပန်း ကြီး",
            'random_small': "ကျပန်း သေး",
            'random_bot': "ကျပန်း ကြီး/သေး",
            'follow_bot': "နောက်ကျော်လိုက်ပါ",
            'bs_formula': "BS ပုံစံ",
            'colour_formula': "အရောင် ပုံစံ",
            'bot_stats': "ဘော့ခ် ကိန်းဂဏန်း",
            'set_bet_sequence': "ချက်ထားမှု အစီအစဉ်",
            'profit_target': "ရန်းပိုက် ရည်မှန်းချက်",
            'loss_target': "ဆုံးရှုံးမှု ရည်မှန်းချက်",
            'reset_stats': "ကိန်းဂဏန်း ပြန်စ",
            'back_main_menu': "မူလ မီနူး",
            'bigwin_login': "လော့ဂ်အင်",
            'results': "ရလဒ်များ",
            'bet_big': "ကြီး ချက်ထားပါ",
            'bet_small': "သေး ချက်ထားပါ",
            'bet_red': "အနီ ချက်ထားပါ",
            'bet_green': "အစိမ်း ချက်ထားပါ",
            'bet_violet': "အပြာ ချက်ထားပါ",
            'my_bets': "ကျွန်ုပ်၏ ချက်များ",
            'sl_layer': "SL Layer",
            'language': "ဘာသာစကား",
            'run_bot': "ဘော့ခ် စပါ",
            'stop_bot': "ဘော့ခ် ရပ်ပါ",
            'bot_info': "ဘော့ခ် အချက်အလက်",
        },
        'chinese': {
            'welcome': "自动彩票机器人\n欢迎!",
            'login_success': "登录成功!",
        ...
        'urdu': {
            ...
            'bet_placed': "شراکت کامیاب!",
            'balance': "بیلنس",
            'language_set': "زبان اردو میں تبدیل کر دی گئی\nاب تمام بوٹ پیغامات اردو میں ظاہر ہوں گے۔",
            'choose_language': "اپنی زبان منتخب کریں\nبراہ کرم اپنی ترجیحی زبان منتخب کریں:",
            'bot_settings': "بوٹ ترتیبات",
            'random_big': "بڑا شرط",
            'random_small': "چھوٹا شرط",
            'random_bot': "رینڈم بٹ",
            'follow_bot': "بوٹ کا تعاقب کریں",
            'bs_formula': "BS فارمولہ",
            'colour_formula': "رنگ کا فارمولہ",
            'bot_stats': "بوٹ کے اعداد و شمار",
            'set_bet_sequence': "شراکت کی ترتیب مقرر کریں",
            'profit_target': "منافع کا ہدف",
            'loss_target': "نقصان کا ہدف",
            'reset_stats': "اعداد و شمار ری سیٹ کریں",
            'back_main_menu': "مرکزی مینو",
            'bigwin_login': "لاگ ان",
            'results': "نتائج",
            'bet_big': "بڑا شرط",
            'bet_small': "چھوٹا شرط",
            'bet_red': "سرخ شرط",
            'bet_green': "سبز شرط",
            'bet_violet': "بنفشی شرط",
            'my_bets': "میری شراکتیں",
            'sl_layer': "SL Layer",
            'language': "زبان",
            'run_bot': "بوٹ چلائیں",
            'stop_bot': "بوٹ بند کریں",
...
class LotteryBot:
    def __init__(self, platform='777'):
        ...
        self.token = "" # ဒါကို အဆုံးမှာထားပါ

            # ... အခြား လုပ်ဆောင်ချက်များ ...

    def sign_md5(self, data_dict): # <--- ဒီလိုင်းမှာ ကော်လံ (:) ရှိရပါမယ်
        """Generate MD5 signature for API requests""" # <--- ဒီလိုင်းကို အနည်းဆုံး ၄ ချက် space သို့မဟုတ် tab နဲ့ indent လုပ်ပေးရပါမယ်
        sign_data = data_dict.copy()
        if 'signature' in sign_data: # <--- ဒီလိုင်းကိုလည်း indent လုပ်ပေးရပါမယ်
            del sign_data['signature'] # <--- ဒီလိုင်းကိုလည်း indent လုပ်ပေးရပါမယ်
        if 'timestamp' in sign_data: # <--- ဒီလိုင်းကိုလည်း indent လုပ်ပေးရပါမယ်
            del sign_data['timestamp'] # <--- ဒီလိုင်းကိုလည်း indent လုပ်ပေးရပါမယ်
        sorted_data = dict(sorted(sign_data.items())) # <--- ဒီလိုင်းကိုလည်း indent လုပ်ပေးရပါမယ်
        hash_string = json.dumps(sorted_data, separators=(',', ':')).replace(' ', '') # <--- ဒီလိုင်းကိုလည်း indent လုပ်ပေးရပါမယ်
        md5_hash = hashlib.md5(hash_string.encode('utf-8')).hexdigest() # <--- ဒီလိုင်းကိုလည်း indent လုပ်ပေးရပါမယ်
        return md5_hash # <--- ဒီလိုင်းကိုလည်း indent လုပ်ပေးရပါမယ်

    def random_key(self): # <--- နောက်ထပ် လုပ်ဆောင်ချက်များကို ဆက်ရေးနိုင်ဖို့အတွက် အလိုအလျောက် class နဲ့ အဆင့်တူအထိ ပြန်ဆင်းပါသည်
        """Generate random key for API""" # <--- ဒီလိုင်းကို အနည်းဆုံး ၄ ချက် space သို့မဟုတ် tab နဲ့ indent လုပ်ပေးရပါမယ်
        xxxx = "xxxxxxxxxxxx4xxxyxxxxxxxxxxxxxxx" # <--- ဒီလိုင်းကိုလည်း indent လုပ်ပေးရပါမယ်
        result = "" # <--- ဒီလိုင်းကိုလည်း indent လုပ်ပေးရပါမယ်
        for char in xxxx: # <--- ဒီလိုင်းကိုလည်း indent လုပ်ပေးရပါမယ်
            if char == 'x': # <--- ဒီလိုင်းကိုလည်း indent လုပ်ပေးရပါမယ်
                result += random.choice('0123456789abcdef') # <--- ဒီလိုင်းကိုလည်း indent လုပ်ပေးရပါမယ်
            elif char == 'y': # <--- ဒီလိုင်းကိုလည်း indent လုပ်ပေးရပါမယ်
                result += random.choice('89a') # <--- ဒီလိုင်းကိုလည်း indent လုပ်ပေးရပါမယ်
            else: # <--- ဒီလိုင်းကိုလည်း indent လုပ်ပေးရပါမယ်
                result += char # <--- ဒီလိုင်းကိုလည်း indent လုပ်ပေးရပါမယ်
        return result # <--- ဒီလိုင်းကိုလည်း indent လုပ်ပေးရပါမယ်
    ...
    if patterns_data['bs_pattern']:
        ...
        for i, bet_type in enumerate(pattern_list):
            if i == current_index:
                ...
                pattern_display += f"{bet_type} "
        await update.message.reply_text(
            f"Current BS Pattern\n\n- BS Formula - Follow BS Pattern (B,S only)\n\nBS Pattern: {patterns_data['bs_pattern']}\nCurrent Position: {current_index}\nProgress: {pattern_display}\nNext bet: {pattern_list[current_index] if current_index < len(pattern_list) else 'Pattern completed - will restart from beginning'}\nBot is following this BS pattern in BS Formula mode."
        )
    else:
        await update.message.reply_text(
...
async def view_colour_pattern_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if patterns_data['colour_pattern']:
        ...
        await update.message.reply_text(
            f"Current Colour Pattern\n\n- Colour Formula - Follow Colour Pattern (G,R,V only)\n\nColour Pattern: {patterns_data['colour_pattern']}\nCurrent Position: {current_index}\nProgress: {pattern_display}\nNext bet: {next_name}\nColour Guide:\n{colour_guide}\nBot is following this Colour pattern in Colour Formula mode."
        )
    else:
        await update.message.reply_text(
...
async def run_sl_bot_integrated(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: str):
    ...
    # FIXED: Bet sequence information မှန်ကန်စွာပြပါ
    bet_sequence = get_user_setting(user_id, 'bet_sequence', '100,300,700,1600,3200,7600,16000,32000')
    current_amount = get_current_bet_amount(user_id)
    await update.message.reply_text(
        f"SL Layer Bot Started!\n\nBS/Colour Pattern Mode: Active\n\nSL Pattern: {sl_pattern_data['pattern']}\nStarting at: SL {sl_pattern_data['current_sl']}\nMode: {mode_text}\nStatus: {status_details}\nBet Sequence: {bet_sequence}\nStarting Bet: {current_amount} K\nBot Status: RUNNING\nBot will now start with {mode_text} mode for SL {sl_pattern_data['current_sl']}.",
        parse_mode='Markdown'
    )
    asyncio.create_task(sl_betting_loop(user_id, context))

async def stop_bot_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Stop auto betting bot immediately - WITH ERROR HANDLING"""
    user_id = str(update.effective_user.id)
    if user_id in auto_betting_tasks:
    ...
    try:
        ...
        logger.error(f"Error clearing pending bets: {e}")
...
async def check_targets(user_id: str, context: ContextTypes.DEFAULT_TYPE):
    ...
    if loss_target > 0 and session_loss >= loss_target:
        await context.bot.send_message(
            chat_id=int(user_id),
            text=f"LOSS TARGET REACHED!\n\nTarget: {loss_target:,} K\nAchieved: {session_loss:,} K\nBot has been stopped automatically.",
            parse_mode='Markdown'
        )
        if user_id in auto_betting_tasks:
            del auto_betting_tasks[user_id]
...
async def view_sl_pattern_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"Current SL Pattern\n\nBS Pattern Mode: {bs_status}\n\nColour Pattern Mode: {colour_status}\n\nSL Pattern: {pattern_text}\n\nCurrent Mode: {mode_status}\n\nProgress:{pattern_display}\n\nCurrent Stats:\n\n- Wait Loss Count: {wait_loss_count}/{pattern_list[current_index] if current_index < len(pattern_list) else pattern_list[-1]}\n\n- Bet Count: {bet_count}/3\n\nNext Action:\n\n{get_next_sl_action(user_id)}",
        parse_mode='Markdown'
    )
    ...
    try:
        ...
        if not sl_session['is_wait_mode']:
            if result == "WIN":
                ...
                next_amount = amounts[0]  # Win ပါက ပုံစံကို ပြန်စပါ
            ...
            # Wait Bot Mode တွင် sequence ကို မပြောင်းပါနဲ့
        ...
        if not sl_session['is_wait_mode']:  # Only update bet count in BETTING mode
            if result == "WIN":
                # Win ပါက Bet Count ကို 0 သို့ ပြန်စပါ (SL Change ဖြစ်မည့်အတွက်)
                new_bet_count = 0
                print(f"DEBUG: WIN - Bet Count reset to 0 (SL Change will happen)")
            else:
                # Loss ပါက Bet Count ကို တိုးပါ
...
Total Profit: {total_profit:,} K
            ...
            if result == "WIN":
                ...
                # FIXED: Win ပါက SL Change ဖြစ်ပါ
                save_sl_bet_session(user_id, is_wait_mode, '', '', 0, 0)
...
async def check_single_bet_result(user_id: str, context: ContextTypes.DEFAULT_TYPE, issue: str, bet_type_str: str, amount: int, platform: str):
    ...
    try:
        ...
        if is_sl_bet:
            ...
            sl_session = get_sl_bet_session(user_id)
            # FIXED: Wait Bot Mode တွင် bet sequence ကို မပြောင်းပါနဲ့
            if not sl_session['is_wait_mode']:
                # Betting Mode တွင်သာ bet sequence update လုပ်ပါ
                current_index = get_user_setting(user_id, 'current_bet_index', 0)
                new_index = update_bet_sequence(user_id, bet_result)
                print(f"DEBUG: Normal Bet - Updated index from {current_index} to {new_index}")
            else:
                # Wait Bot Mode တွင် bet sequence ကို မပြောင်းပါနဲ့
                current_index = get_user_setting(user_id, 'current_bet_index', 0)
            ...
            if bet_result == "WIN":
                ...
                # Wait Bot Mode တွင် sequence info ကို မပြပါနဲ့
                if sl_session['is_wait_mode']:
                    sequence_info = "Status: Wait Bot Mode - Sequence Frozen"
                else:
                    current_index = get_user_setting(user_id, 'current_bet_index', 0)
...
{pattern_info}
        ...
        # FIXED: Wait Bot Mode တွင် waiting_for_results ကို False ပြန်ပြောင်းပါ
        waiting_for_results[user_id] = False
    else:
...
async def set_burmese_language(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "ဘာသာစကားကို မြန်မာဘာသာသို့ ပြောင်းလိုက်ပါပြီ\n\nအားလုံးကို မြန်မာဘာသာဖြင့် ပြပေးမည်ဖြစ်ပါသည်။",
        reply_markup=get_main_keyboard(user_id)
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # အသုံးပြုသူစာရင်းထည့်တဲ့အမိန့်
    if text.startswith('/adduser') and user_id == '6328953001': # Admin ID ကို စာရင်းထဲမှာ ပါပါ
        try:
            # အသစ်ထည့်မယ့် User ID ကိုဖတ်ပါ
            new_user_id = int(text.split()[1])
            if new_user_id not in ALLOWED_USER_IDS:
                ALLOWED_USER_IDS.append(new_user_id)
                # Save to database as well
                add_user_to_db(new_user_id)
                await update.message.reply_text(f"အသုံးပြုသူ ID {new_user_id} ကို စာရင်းတွင်ထည့်ပြီးပါပြီ။")
            else:
                await update.message.reply_text(f"အသုံးပြုသူ ID {new_user_id} သည် စာရင်းတွင် ပါဝင်ပြီးဖြစ်ပါသည်။")
        except (IndexError, ValueError):
            await update.message.reply_text("အသုံးပြုသူ ID ကိုမှန်ကန်စွာ ထည့်ပေးပါ။ ဥပမာ: /adduser 987654321")
        return # အခြားအမိန့်များကို မလုပ်တော့ပါနဲ့

    # အသုံးပြုသူစာရင်းဖျက်တဲ့အမိန့်
    if text.startswith('/removeuser') and user_id == '6328953001': # Admin ID ကို စာရင်းထဲမှာ ပါပါ
        try:
            remove_user_id = int(text.split()[1])
            if remove_user_id in ALLOWED_USER_IDS:
                ALLOWED_USER_IDS.remove(remove_user_id)
                # Remove from database as well
                remove_user_from_db(remove_user_id)
                await update.message.reply_text(f"အသုံးပြုသူ ID {remove_user_id} ကို စာရင်းမှဖျက်ပြီးပါပြီ။")
            else:
                await update.message.reply_text(f"အသုံးပြုသူ ID {remove_user_id} သည် စာရင်းတွင် မပါဝင်ပါ။")
        except (IndexError, ValueError):
            await update.message.reply_text("အသုံးပြုသူ ID ကိုမှန်ကန်စွာ ထည့်ပေးပါ။ ဥပမာ: /removeuser 987654321")
        return # အခြားအမိန့်များကို မလုပ်တော့ပါနဲ့

    # အသုံးပြုသူစာရင်းကြည့်တဲ့အမိန့်
    if text == '/listusers' and user_id == '6328953001': # Admin ID ကို စာရင်းထဲမှာ ပါပါ
        user_list = "\n".join([f"- {uid}" for uid in ALLOWED_USER_IDS])
        await update.message.reply_text(f"အသုံးပြုခွင့်ပေးထားသော အသုံးပြုသူများ:\n{user_list}")
        return # အခြားအမိန့်များကို မလုပ်တော့ပါနဲ့
    ...
    if user_session['step'] == 'login_phone':
        ...
        try:
            ...
            if target_amount == 0:
                ...
                await update.message.reply_text(
                    f"Profit target set to: {target_amount:,} K\n\nBot will automatically stop when profit reaches {target_amount:,} K",
                    reply_markup=get_bot_settings_keyboard(user_id)
                )
        except ValueError:
        ...
        try:
            ...
            if target_amount == 0:
                ...
                await update.message.reply_text(
                    f"Loss target set to: {target_amount:,} K\n\nBot will automatically stop when loss reaches {target_amount:,} K",
                    reply_markup=get_bot_settings_keyboard(user_id)
                )
        except ValueError:
        ...
        if all(c in valid_chars for c in pattern.replace(' ', '')):
            ...
            if save_formula_patterns(user_id, colour_pattern=clean_pattern):
                ...
                await update.message.reply_text(
                    f"Colour Pattern Set Successfully!\n\n- Colour Formula - Follow Colour Pattern (G,R,V only)\n\nColour Pattern: {clean_pattern}\nColour Bets: {colour_count}/{total_bets}\nStarting from first position.\n\nPattern Guide:\n\n- R = RED\n\n- G = GREEN\n\n- V = VIOLET\n\nBot will now follow this Colour pattern in Colour Formula mode.\n\nNote: Only G (GREEN), R (RED), and V (VIOLET) are allowed in Colour Formula.",
    ...
    # ===== LOGIN MENU HANDLING =====
        ...
        # Check if the current user's ID is in the allowed list
        if current_user_id in ALLOWED_USER_IDS:
            # IDs match, proceed with the normal login process
            await process_login(update, context, save_credentials=True)
        else:
            # IDs don't match, send the error message and contact info
            await update.message.reply_text(
                f"ID မှားနေရင် Admin Contact @Smile_p2 ပြပေးပါ\n"
                f"သင့် Telegram User ID ({current_user_id}) ကို အသုံးပြုခွင့် မပေးထားပါ။"
            )
        # Do not proceed to the original process_login call if the ID doesn't match
        return # အသုံးပြုခွင့်မပေးရင် ဒီနေရာနဲ့ ရပ်ပါ
...
def main():
    ...
    # Load allowed users from database on startup
    global ALLOWED_USER_IDS
    ALLOWED_USER_IDS = load_allowed_users_from_db()
    print(f"ALLOWED_USER_IDS loaded from DB: {ALLOWED_USER_IDS}")
    ...
    print("NEW: Win ပြန်ပေးခြင်းနှင့် SL ပြောင်းခြင်း")
    print("FIXED: SL Bot Bet Count နှင့် Bet Result Update ပြဿနာများ")
    print("FIXED: 'already processed' issue for SL Bot Win/Loss messages")
    print("FIXED: Current Step 1 မှာ Loss ဖြစ်ပါက Current Step 2 သို့ မှန်ကန်စွာ ပြောင်းခြင်း")
    print("FIXED: Bet Sequence ပြဿနာများ - 10K မှ 30K သို့ မှန်ကန်စွာ တိုးခြင်း")
